...
class RWQueue {
	...
	public:
		...
		void setNext(QueueNode* next) {
			...
			bool succ = false;
			while (!succ) {
				QueueNode* old = nextitem.load(memory_order_acquire);
	...
	void enqueue(RWnode* o) {
		...
		while (!succ1 || !succ2) {
			if (first.load(memory_order_relaxed)->getItem() == NULL) {
				...
				if (first.compare_exchange_weak(old, old->getNext(), memory_order_release, memory_order_relaxed)) {
					succ1 = true;
				...
				QueueNode* last_old = first.load(memory_order_acquire);
				if (last.compare_exchange_weak(last_old, first, memory_order_release, memory_order_relaxed)) {
	...
	RWnode * dequeue() {
		...
		while (!succ1 || !succ2) {
			...
			if (first.load(memory_order_acquire) == last.load(memory_order_acquire)) {
				...
				if (last.compare_exchange_weak(old, temp, memory_order_release, memory_order_relaxed)) {
					size.fetch_add(1, memory_order_relax ed);
					succ1 = true;
				}
				if (first.compare_exchange_weak(first_old, temp, memory_order_release, memory_order_relaxed)) {
					size.fetch_add(1, memory_order_relax ed);
					succ2 = true;
				}
